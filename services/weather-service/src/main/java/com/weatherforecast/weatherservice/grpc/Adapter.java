...
/**
 * The Adapter class provides utility methods to convert between domain models
 * and gRPC protocol buffers.
 * It contains methods to transform WeatherData domain objects to gRPC responses
 * and gRPC requests to
 * coordinate data used by the application.
 *
 * <p>
 * This class serves as a bridge between the domain layer and the gRPC
 * communication layer,
...
public class Adapter {
    public static WeatherDataResponse toGrpcResponse(WeatherData weatherData) {
        WeatherDataResponse.Builder responseBuilder = WeatherDataResponse.newBuilder()
        ...
        if (weatherData.getForecast() != null) {
            for (Forecast forecast : weatherData.getForecast()) {
                ForecastData forecastData = ForecastData.newBuilder()
        ...
        if (weatherData.getAlerts() != null) {
            for (Alert alert : weatherData.getAlerts()) {
                AlertData alertData = AlertData.newBuilder()
