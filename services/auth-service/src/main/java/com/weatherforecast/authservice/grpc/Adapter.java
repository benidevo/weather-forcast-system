...
/**
 * Adapter class that provides conversion methods between domain objects and
 * gRPC protocol objects.
 *
 * The adapter
 * methods transform internal domain objects into their corresponding gRPC
 * message formats that
 * are suitable for network transmission.
 *
 * It contains utility methods for converting authentication and
 * registration related
 * domain objects to their gRPC counterparts, helping to isolate domain logic
 * from the
 * transport-specific details of the gRPC implementation.
 */
public class Adapter {

    /**
     * Converts an AuthToken object to its gRPC AuthResponse representation.
     * This adapter method facilitates the communication between the application
     * domain model
     * and the gRPC service layer.
     *
     * @param authToken The authentication token object from the application domain
     * @return A gRPC AuthResponse object containing the success status, token
     *         string,
    ...
    public static AuthResponse toGrpcAuthResponse(AuthToken authToken) {
        var response = AuthResponse.newBuilder()
                .setSuccess(authToken.isSuccess());
        if (authToken.isSuccess()) {
            response
                    .setToken(authToken.getToken())
                    .setExpiresAt(authToken.getExpiresAt().toString());
        }
    ...
    /**
     * Converts a boolean success status to its gRPC RegistrationResponse
     * representation.
     * This adapter method facilitates the communication between the application
     * domain model
     * and the gRPC service layer.
