...
class AuthServiceImplTest {
  ...
  @BeforeEach
  void setUp() {
    testUser = User.builder().username("testuser").password("encodedPassword").build();
  ...
  void updateUsername_WhenNewUsernameSameAsOld_ShouldReturnFalse() {
    when(userRepository.findByUsername("olduser"))
        .thenReturn(Optional.of(User.builder().username("olduser").build()));
    when(userRepository.findByUsername("olduser")).thenReturn(Optional.empty());
